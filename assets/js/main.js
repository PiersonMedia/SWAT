...
const EXFIL_POINT = new THREE.Vector3(-6.0, 1.7, -16.6);
const EXFIL_RADIUS = 3;
...
function init() {
    ...
    if (deployBtn) deployBtn.addEventListener('click', deployMission);
    const abortBtn = document.getElementById('abort-btn');
    if (abortBtn) abortBtn.addEventListener('click', abortBriefing);
    ...
    if (intersects.length > 0) {
        ...
        if (hitObject.userData.isEnemy && hitObject.userData.isAlive) {
            ...
            if (hitObject.userData.health <= 0) {
                ...
                pushStatusFeed('SUSPECT NEUTRALIZED');
                updateMissionStatus();
            }
...
function rescueHostage(hostage) {
    ...
    gameState.score += 500;
    pushStatusFeed('HOSTAGE SECURED');
    updateMissionStatus();
...
function giveOrder(order) {
    ...
    pushStatusFeed(orders[order] || order.toUpperCase());
}
...
function checkMissionComplete() {
    if (gameState.mode === 'hostage') {
        if (gameState.hostagesRescued >= gameState.hostagesTotal) {
            ...
            pushStatusFeed('COMMAND: All hostages secured. Proceed to exfil.');
        }
    }
}
...
function takeDamage(amount) {
    ...
    if (gameState.health <= 0) {
        ...
        pushStatusFeed('COMMAND: Operator down. Mission failed.');
        showDebrief();
    }
}
...
function checkExfil() {
    if (gameState.phase !== 'exfil') return;
    const distance = camera.position.distanceTo(EXFIL_POINT);
    if (distance <= EXFIL_RADIUS) {
        gameState.phase = 'complete';
        showNotification('EXFIL COMPLETE - MISSION SUCCESS', '#00ff88');
        pushStatusFeed('COMMAND: Team exfil complete.');
        showDebrief();
    }
}
...
function updateMissionStatus() {
    const hostages = document.getElementById('status-hostages');
    const suspects = document.getElementById('status-suspects');
    const civilians = document.getElementById('status-civilians');
    const score = document.getElementById('status-score');

    if (hostages) hostages.textContent = `${gameState.hostagesRescued} / ${gameState.hostagesTotal}`;
    if (suspects) suspects.textContent = `${gameState.suspectsNeutralized} / ${gameState.suspectsTotal}`;
    if (civilians) civilians.textContent = `${gameState.civiliansHarmed}`;
    if (score) score.textContent = `${gameState.score}`;
}

function updateScoreboard() {
    const timeEl = document.getElementById('score-time');
    const hostagesEl = document.getElementById('score-hostages');
    const suspectsEl = document.getElementById('score-suspects');
    const civiliansEl = document.getElementById('score-civilians');
    const totalEl = document.getElementById('score-total');

    if (timeEl) timeEl.textContent = formatMissionTime();
    if (hostagesEl) hostagesEl.textContent = `${gameState.hostagesRescued} / ${gameState.hostagesTotal}`;
    if (suspectsEl) suspectsEl.textContent = `${gameState.suspectsNeutralized} / ${gameState.suspectsTotal}`;
    if (civiliansEl) civiliansEl.textContent = `${gameState.civiliansHarmed}`;
    if (totalEl) totalEl.textContent = `${gameState.score}`;
}

function pushStatusFeed(message) {
    const list = document.getElementById('status-feed-list');
    if (!list) return;
...
function setupBriefing(type) {
    const briefingData = {
        hostage: {
            title: 'OPERATION NIGHTFALL',
            subtitle: 'GAS STATION HOSTAGE RESCUE',
            objectives: [
                'Rescue all hostages.',
                'Neutralize armed suspects.',
                'Secure the back office.'
            ],
            intel: 'Suspects barricaded inside the gas station. Hostages last seen in the back office. Expect limited visibility.'
        },
        shoot_house: {
            title: 'SHOOT HOUSE',
            subtitle: 'TRAINING CLEARANCE RUN',
            objectives: ['Clear all hostile targets.', 'Avoid civilian silhouettes.'],
            intel: 'Live-fire drill. Maintain muzzle discipline and clear each room with intent.'
        },
        breaching: {
            title: 'BREACHING DRILL',
            subtitle: 'ENTRY & CLEAR',
            objectives: ['Practice door breaching.', 'Use flashbangs to disorient targets.'],
            intel: 'Simulated suspect positions. Focus on flow and communication.'
        }
    };

    const data = briefingData[type] || briefingData.hostage;
    const titleEl = document.getElementById('briefing-title');
    const subtitleEl = document.getElementById('briefing-subtitle');
    const objectivesEl = document.getElementById('briefing-objectives');
...
function showBriefing(type) {
    ...
    setupBriefing(type);
    const feedList = document.getElementById('status-feed-list');
...
function deployMission() {
    ...
    createEnvironment(gameState.mode);
    setupWeapon();

    player.x = EXFIL_POINT.x;
    player.y = EXFIL_POINT.y;
    player.z = EXFIL_POINT.z;
    ...
    pushStatusFeed('COMMAND: Team deployed. Clear the structure.');
    updateAmmoDisplay();
    updateHealthDisplay();
    updateStanceDisplay();
    updateMissionStatus();
...
function toggleScoreboard() {
    ...
    scoreboard.classList.toggle('hidden', !scoreboardVisible);
...
function startMission(type) {
    ...
    gameState.score = 0;
    gameState.hostagesTotal = type === 'hostage' ? 3 : 0;
    gameState.suspectsTotal = 5;
